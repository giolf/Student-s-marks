...
import static ch.studentgrades.DataSource.getPosizioneAnno;
import static ch.studentgrades.DataSource.setNomeAnnoSelezionato;
import static ch.studentgrades.Utility.customTitleDialog;
import static ch.studentgrades.Utility.makeFrameLWithNumPicker;
import static ch.studentgrades.Utility.setupMenuRiepilogoAnnuale;
...
public class CorsiAnnuali extends Fragment {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if ( !activity.getMNavigationDrawerFragment().isDrawerOpen() ) {
            ...
            //se non ce selezionato nessun anno avverto l'utente che non è possibile visualizzare la lista dei corsi
            if ( nomeAnnoSelezionato.equals("-") ) {
                //trovarsi dentro a questo blocco significa, indirettamente, a non aver creato nessun anno
                rootView.findViewById(R.id.lwCorsiAnnuali).setVisibility(View.INVISIBLE);
                rootView.findViewById(R.id.msgMediaCorsiNonDisp).setVisibility(View.VISIBLE);
                //dopo di che termino qui il metodo
                return;
            }
            //se l'anno selezionato non ha voti avverto l'utente che non è possibile visualizzare la media dei corsi
                ...
                //recupero la lista dei corsi che hanno almeno 1 voto
                ArrayList<Corso> listaCorsi = anno.creaListaCorsiConVoti();

                //nascondo il messaggio che indica che non ci sono corsi disponibili per l'anno selezionato
                rootView.findViewById(R.id.msgMediaCorsiNonDisp).setVisibility(View.INVISIBLE);

                //setto l'adapter alla listView con la lista dei corsi dell'anno selezionato
            ...
            if (getListaAnni().size() == 0 ) {
                ...
                Dialog.OnClickListener seleziona = new Dialog.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        //se l'anno selezionato non ha voti avverto l'utente che non è possibile visualizzare la lista dei corsi
                        if (annoSelezionato.numeroVotiAnnuali() == 0) {
                            ...
                            ListView lw = (ListView)rootView.findViewById(R.id.lwCorsiAnnuali);
                            AdapterListaCorsiView adapter =(AdapterListaCorsiView)lw.getAdapter();
                            adapter.aggiornaListaCorsi(listaCorsi);

                            //infine rendo visibile la listView che visualizzerà la lista dei corsi dell'anno selezionato
                            //e per sicurezza nascondo la textview del messaggio 'media corsi non disponibile' (in quanto potrebbe essere ancora visibile)
                            rootView.findViewById(R.id.msgMediaCorsiNonDisp).setVisibility(View.INVISIBLE);
                            rootView.findViewById(R.id.lwCorsiAnnuali).setVisibility(View.VISIBLE);
                        }
                    }
                };
